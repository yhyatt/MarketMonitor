...
class DigestFormatter:
    ...
    def format(
        ...
        aitldr_items: list[dict] | None = None,
        ...
        telegram = self._format_telegram(papers, hf_items, github_signals, aitldr_items, weekly_synthesis, date_str)
        html = self._format_html(papers, hf_items, github_signals, aitldr_items, weekly_synthesis, week_str)
        plain = self._format_plain(papers, hf_items, github_signals, aitldr_items, weekly_synthesis, week_str)
...
"""
        ...
        # AI/TLDR section
        if aitldr_items:
            html += """<h2 style="color: #4f46e5; font-size: 18px; margin-top: 24px;">📡 AI/TLDR Weekly Digest</h2>"""
            for item in aitldr_items[:10]:
                title = item.get("title", "")
                org = item.get("org", "")
                url = item.get("url", "")
                ...
                border_color = "#ef4444" if importance == "seismic" else "#f97316" if importance == "major" else "#8b5cf6"
                badge = f"<span style='background:{border_color};color:#fff;padding:2px 6px;border-radius:4px;font-size:11px;margin-left:8px;'>{importance.upper()}</span>" if importance else ""

                html += f"""
<div style="margin-bottom: 12px; padding: 10px; background-color: #faf5ff; border-radius: 6px; border-left: 3px solid {border_color};">
<strong><a href="{url}" style="color: #7c3aed; text-decoration: none;">{title}</a></strong>{badge}
<span style="color: #6b7280; font-size: 13px;"> — {org}</span>
<p style="margin: 4px 0 0 0; color: #475569; font-size: 14px;">{summary[:200]}</p>
</div>
"""
...
Generated by market-monitor • <a href="https://github.com/openclaw/openclaw" style="color: #6366f1;">OpenClaw</a>
    ...
    def _format_plain(
        ...
        if aitldr_items:
            lines.append("AI/TLDR WEEKLY DIGEST")
            lines.append("-" * 20)
            for item in aitldr_items[:10]:
                title = item.get("title", "")
                org = item.get("org", "")
                url = item.get("url", "")
                importance = item.get("importance", "")
                badge = f" [{importance.upper()}]" if importance else ""
                lines.append(f"• {title}{badge} — {org}")
                if url:
                    lines.append(f"  {url}")
            lines.append("")
